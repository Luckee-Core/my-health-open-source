...
const GroupDescription = (props: GroupDescriptionProps) => {
 ...
 */
export const ApiDocsContent = (props: Props) => {
  const { catalog } = props;
            ...
            {isOverview ? (
              <>
                ...
                <p className={styles.envelope}>
                  Response envelope: <code className={styles.code}>{catalog.responseEnvelope}</code>
                </p>
                {group.description ? (
                  <GroupDescription description={group.description} className={styles.overviewDesc} />
                ) : null}
              </>
            ) : (
              <>
            ...
            {group.endpoints.length > 0 ? (
              <div className={styles.endpoints}>
                {group.endpoints.map((endpoint) => {
                  ...
                  return <EndpointCard key={anchorId} endpoint={endpoint} anchorId={anchorId} />;
                })}
              </div>
            ) : null}
          </section>
...
const styles = {
  ...
  `,
  code: `
    rounded bg-muted px-1 py-0.5 text-xs font-mono text-foreground
  `,
  group: `
    scroll-mt-8
  `,

...
- Reads **one** whole current slice: `useAppSelector((state) => state.currentHospital)`
...
**Modals take no props** — not the entity, not `isOpen`, not `onClose`. Visibility is `builder.isCreateOpen || current.id !== ''` (plus `isEditOpen` when that flag exists). Close dispatches `closeModal()` and `resetCurrentX()`. The package page renders `<HospitalFormModal />` only.
...
Related-row creates (e.g. new hospital name while editing a doctor) read **builder strings**, not a form payload object.

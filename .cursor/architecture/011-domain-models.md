...
Persisted health-record entities are the contract between Express JSON and Redux dumps. They must not live in API clients, packages, or store slices. ADR 005 omitted `src/model` from the canonical tree even though the app already uses it.
...
| `src/packages/{feature}/` | View rows (`FeedDayRow`, `TodayTherapyRow`) |
| `src/store/` | Redux-only UI state (`HealthImportStep`) |
| `src/api/types.ts` | Transport (`ApiResponse`) |

### 4) Alignment with Express

Express stores the same entities in `src/model/` (companion repo ADR 015). File names and row fields should match.

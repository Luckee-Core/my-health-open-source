...
```ts
// src/store/thunks/lots/load-lots-thunk.ts
import type { AppThunk } from '@/store';
import { mapApiFailureToThunkStatus } from '@/api/_shared';
import { getLotsBySellerId } from '@/api/lot';
import { LotsActions } from '@/store/dumps/lots';

/**
 * Loads lots for the current seller through the API layer.
 */
export const loadLotsThunk =
  (sellerId: string): AppThunk<Promise<200 | 400 | 500>> =>
  async (dispatch) => {
    const result = await getLotsBySellerId(sellerId);
    if (!result.success || !result.data) {
      return mapApiFailureToThunkStatus(result);
    }
    dispatch(LotsActions.setLots(result.data));
    return 200;
  };
```
...
## 8) Pairing with express-server-template

When this app talks to a companion **Express** API (e.g. from `express-server-template`):

1. Add **`src/api/{domain}/`** modules that return typed data; call them from **thunks** only (client) or **Server Components** (published reads — see [010 – Public content reads from Express](./010-public-blog-express-fetch.md)).
2. Prefer **`NEXT_PUBLIC_*_EXPRESS_URL`** (or equivalent) in `src/api/config.ts`; never embed secrets in client bundles.
